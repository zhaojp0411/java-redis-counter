...
public class HBaseCounterTest {
	...
	public void setUp() throws Exception {
		...
		counter = new HBaseCounter();
		
		Configuration config = HBaseConfiguration.create();
		HTable counterTable = new HTable(config, HBaseCounter.defaultStrTableName);
		
		Delete delOpt = new Delete("item".getBytes());
		counterTable.delete(delOpt);
		
		for(int i=0;i<10;i++){
			keys.add(("item"+i));
			Delete del = new Delete(("item"+i).getBytes());
			counterTable.delete(del);
		}
